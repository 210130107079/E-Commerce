...
const Add = ({token}) => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
        ...
        <div>
            ...
            <div className='flex gap-3'>
                <div onClick={(e)=>setSizes(prev => prev.includes("S") ? prev.filter(item => item !== "S") : [...prev,"S"])}>
                    <p className={`${sizes.includes("S") ? "bg-green-200" : "bg-slate-200" } cursor-pointer px-3 py-1`}>S</p>
                </div>

                <div onClick={(e)=>setSizes(prev => prev.includes("M") ? prev.filter(item => item !== "M") : [...prev,"M"])}>
                    <p className={`${sizes.includes("M") ? "bg-green-200" : "bg-slate-200" } cursor-pointer px-3 py-1`}>M</p>
                </div>

                <div onClick={(e)=>setSizes(prev => prev.includes("L") ? prev.filter(item => item !== "L") : [...prev,"L"])}>
                    <p className={`${sizes.includes("L") ? "bg-green-200" : "bg-slate-200" } cursor-pointer px-3 py-1`}>L</p>
                </div>

                <div onClick={(e)=>setSizes(prev => prev.includes("XL") ? prev.filter(item => item !== "XL") : [...prev,"XL"])}>
                    <p className={`${sizes.includes("XL") ? "bg-green-200" : "bg-slate-200" } cursor-pointer px-3 py-1`}>XL</p>
                </div>

                <div onClick={(e)=>setSizes(prev => prev.includes("XXL") ? prev.filter(item => item !== "XXL") : [...prev,"XXL"])}>
                    <p className={`${sizes.includes("XXL") ?  "bg-green-200" : "bg-slate-200" } cursor-pointer px-3 py-1`}>XXL</p>
                </div>
        ...
        <button disabled={loading} className='w-40 py-3 mt-4 bg-black text-white hover:text-black hover:bg-white transition-all duration-300' type="submit">
            {
